...
from zwaf.api.routes import health, webhook, payment_webhook, superfrete_webhook, improvements, ab_test
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    for tenant_id, team in teams.items():
        ...
        register_pix_reengagement_scheduler(
            agno_app=app,
            db_url=db_url,
            tenant_id=tenant_id,
            whatsapp_tool=whatsapp_tool,
    ...
    # Shutdown: parar schedulers de expiracao de reservas
    for tenant_id, team in teams.items():
        try:
            scheduler = getattr(team, "_inventory_sweep_scheduler", None)
            if scheduler:
                scheduler.stop()
        except Exception:
            pass
...
app.include_router(improvements.router, prefix="/v1/admin", tags=["Admin"])
app.include_router(ab_test.router, prefix="/v1/admin", tags=["Admin"])
